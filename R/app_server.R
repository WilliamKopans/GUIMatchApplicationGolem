...
app_server <- function( input, output, session ) {
  ...
  observeEvent(input$FinalShowTopPlot, {
    ...
    output$FullTopPlot <- renderPlot({
      ggplot()+
        geom_line(StructuredTopData(), mapping = aes_string(x= input$dynamicTopX, y = input$dynamicTopY)) +
        geom_point(StructuredTopData(), mapping = aes_string(x= input$dynamicTopX, y = input$dynamicTopY),alpha = 0.3) +
        theme_bw()+
        coord_cartesian(ylim = c(input$YRangeTop[1], input$YRangeTop[2]), xlim = c(input$XrangeTop[1], input$XrangeTop[2]), expand = FALSE) + TopGeom()
  ...
  TopGeom <- reactive({
    ...
    if (isTruthy(input$files) == TRUE && isTruthy(input$Bottom$datapath) == TRUE && isTruthy(input$Top$datapath) == TRUE) {
      if (length(parseFilePaths(roots, input$files)$datapath)!=0) { #New tie file import
        ...
        RowsWithNA <- total[rowSums(is.na(total)) > 0,]
        print(typeof(RowsWithNA))
        print(RowsWithNA)
        print(typeof(total))
        print(total)
        ...
        if (nrow(total)>0) {
          ...
          if (nrow(RowsWithNA) < 1) {
            
            
            INDEXFin <- which(colnames(total)==input$dynamicTopY)
            INDEXFinSh <- which(colnames(total)=="Shared1")
            TopGeom <- list(geom_point(data = total, mapping = aes(x = total[,INDEXFinSh], y = total[,INDEXFin]), color ='dodgerblue', shape = 13, size = 9),
                            geom_label_repel(aes(x = total[,INDEXFinSh], y = total[,INDEXFin], label = total$ID), box.padding   = 0.35,  point.padding = 0.5, segment.color = 'grey50'),
                            geom_point(data = total, mapping = aes(x = total[,INDEXFinSh], y = total[,INDEXFin]), color ='dodgerblue', shape = 18, size = 3.5)
            )
            return(TopGeom)
            
          } else {
            print("Entered NA Zone")
            RowsWithoutNA <- total[!rowSums(is.na(total)) > 0,] #Without NA
            RowsWithNA <- total[rowSums(is.na(total)) > 0,] #With NA
            print(head(RowsWithNA))
            
            INDEXFin <- which(colnames(RowsWithoutNA)==input$dynamicTopY)
            INDEXFinSh <- which(colnames(RowsWithoutNA)=="Shared1")
            TopGeom <- list(geom_point(data = RowsWithoutNA, mapping = aes(x = RowsWithoutNA[,INDEXFinSh], y = RowsWithoutNA[,INDEXFin]), color ='dodgerblue', shape = 13, size = 9),
                            geom_label_repel(aes(x = RowsWithoutNA[,INDEXFinSh], y = RowsWithoutNA[,INDEXFin], label = RowsWithoutNA$ID), box.padding   = 0.35,  point.padding = 0.5, segment.color = 'grey50'),
                            geom_point(data = RowsWithoutNA, mapping = aes(x = RowsWithoutNA[,INDEXFinSh], y = RowsWithoutNA[,INDEXFin]), color ='dodgerblue', shape = 18, size = 3.5),
                            geom_label_repel(aes(x = RowsWithNA[,INDEXFinSh], y = RowsWithNA[,INDEXFin], label = RowsWithNA$ID, fill = "red"), box.padding   = 0.35,  point.padding = 0.5, segment.color = 'grey50'),
            )
    ...
    if (isTruthy(input$files) == TRUE && isTruthy(input$Bottom$datapath) == TRUE && isTruthy(input$Top$datapath) == TRUE) {
      if (length(parseFilePaths(roots, input$files)$datapath)!=0) { #New tie file import
        ...
        if (nrow(total)>0) {
          ...
          INDEXFinSh <- which(colnames(total)=="Shared1")
          BottomGeom <- list(geom_point(data = total, mapping = aes(x = total[,INDEXFinSh], y = total[,INDEXFin]), color ='dodgerblue', shape = 13, size = 9),
                             geom_label_repel(aes(x = total[,INDEXFinSh], y = total[,INDEXFin], label = total$ID), box.padding   = 0.35,  point.padding = 0.5, segment.color = 'grey50'),
                             geom_point(data = total, mapping = aes(x = total[,INDEXFinSh], y = total[,INDEXFin]), color ='dodgerblue', shape = 18, size = 3.5)
          )
  ...
  observeEvent(input$FinalShowBottomPlot, {
    output$FullBottomPlot <- renderPlot({
      ...
      ggplot()+
        geom_line(StructuredBottomData(), mapping = aes_string(x= input$dynamicBottomX, y = input$dynamicBottomY)) +
        geom_point(StructuredBottomData(), mapping = aes_string(x= input$dynamicBottomX, y = input$dynamicBottomY),alpha = 0.3) +
        theme_bw() + 
        coord_cartesian(ylim = c(input$YRangeBot[1], input$YRangeBot[2]), xlim = c(input$XRangeBot[1], input$XRangeBot[2]), expand = FALSE)+ BottomGeom()
